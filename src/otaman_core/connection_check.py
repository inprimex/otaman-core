...
# generator READS the last persisted report, joined on connection name. Core
# owns the serialization so the writer and reader cannot diverge.
#
# Location + shape are the contract cli-agent (writer, §3.1) froze and core
# encodes here so both sides call the same code:
#   - Path: ~/.otaman/connection-checks.json — the TENANT home (a directory by
#     construction; one OS user == one tenant; sits with edition.yaml/hitl.yaml).
#     NOT XDG_RUNTIME_DIR (cleared on reboot — a stale last-check still beats
#     nothing). NOT <program_root>/.otaman/... (the `.otaman` marker can be a
#     FILE — you can't nest JSON under a file; that collision bit us earlier).
#   - Envelope: {"version": 1, "programs": {"<project>": [CheckReport, ...]}}
#     keyed by platform.yaml `project:` name, so one tenant file holds every
#     program and joins avoid abspath fragility across checkouts/hosts.
#
# CheckReport carries no secret value, so the store is values-free by
# construction (Q5).
# ---------------------------------------------------------------------------
...
def report_store_path(home: Path | None = None) -> Path:
    """Canonical persisted-report path: ``~/.otaman/connection-checks.json``.

    Tenant-global (one file per OS user), program-keyed inside. ``home`` defaults
    to ``Path.home()`` and is injectable for tests. Durable machine state written
    only by ``otaman connection check`` / ``check --fix``.
    """
    base = home or Path.home()
    return base / ".otaman" / DEFAULT_REPORTS_FILENAME
...
def _load_envelope(path: Path) -> dict:
    """Read the raw ``{"version", "programs": {...}}`` envelope (empty on error)."""
    if not path.is_file():
        return {}
    try:
...
def load_reports(path: Path, program: str) -> dict[str, CheckReport]:
    """Load one program's persisted reports, keyed by connection ``name``.

    Returns an empty dict when the store is absent/malformed OR the program has
    no entry — the generator then renders ``last-check`` as ``—`` for every
    connection. Unknown/extra fields are ignored (forward-compatible).
    """
    programs = _load_envelope(path).get("programs", {})
    if not isinstance(programs, dict):
        return {}
    entries = programs.get(program, [])
...
def persist_reports(
    reports: Iterable[CheckReport], path: Path, program: str
) -> dict[str, CheckReport]:
    """Upsert ``reports`` into ``programs[program]`` keyed by name; atomic 0600 write.

    A single ``otaman connection check <name>`` updates just that connection's
    entry; ``check --all`` refreshes them all. Other connections in the program
    AND other programs in the tenant file are preserved. Returns the program's
    full merged map. Values-free by construction (CheckReport has no value field).
    """
    envelope = _load_envelope(path)
    programs = envelope.get("programs")
    if not isinstance(programs, dict):
        programs = {}
    merged = load_reports(path, program)
    for report in reports:
        merged[report.name] = report
    programs[program] = [asdict(r) for r in sorted(merged.values(), key=lambda r: r.name)]
    payload = {"version": REPORT_STORE_VERSION, "programs": programs}
